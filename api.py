...
async def no_knowledge_chat(
        prompt: str = Body(..., description="Question", example="工伤保险是什么？"),
        history: List[List[str]] = Body(
            [],
            description="History of previous questions and answers",
            example=[
    ...
    for resp, history in local_doc_qa.get_no_knowledge_based_answer(
            query=prompt, chat_history=history, streaming=True
    ):
        pass
    return ChatMessage(
        question=prompt,
...
def main():
    ...
    app.websocket("/chat-docs/stream-chat/{session_id}")(stream_chat)
